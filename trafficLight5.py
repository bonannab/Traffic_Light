...
for j in imgs:
    ...
    for i in data[k].get("labels"):
        if i.get("category") == "traffic light":
            ...
            cv2.imshow("traffic light" + str(i), tl)
            ...
            IMAGE_LIST.append((tl,i.get("attributes")["trafficLightColor"]))
    k = k + 1
...
#image2, label2, index3 = getImage(color=IMAGE_LIST[1][1], Random=True, showImg=True)###########################
image2, label2, index3 = getImage(color='red', Random=True, showImg=True)
...
#random_red, _, index1 = getImage(color=IMAGE_LIST[1][1], Random=True)#########################xx
#random_yellow, _, index2 = getImage(color=IMAGE_LIST[1][1], Random=True)########################xxx
#random_green, _, index3 = getImage(color=IMAGE_LIST[1][1], Random=True)##########################x
...
# Print image label
print('Label [off, red, yellow, red-yellow green]: ' + str(test_label))
...
def estimate_label(rgb_image, print_stats=False):
    ...
    # choose color if available
        ...
        if has_label(predicted_label_color) and predicted_label_color != [0, 0, 1, 0, 0]:
            predicted_label = predicted_label_color
...
# Using the load_dataset function in helpers.py
# Load test data
#TEST_IMAGE_LIST = helpers.load_dataset('../scratch/fs2/DTLD_final/Bochum/Bochum1/2015-04-21_17-09-21/2')###################x
...
def get_misclassified_images(test_images):
    ...
    for image in test_images:
        ...
        assert (len(predicted_label) == 5), "The predicted_label is not the expected length (5)."
        ...
        if redd < 100 and yelloww < 100 and greenn < 100:
            predicted_label = [1, 0, 0, 0, 0]
        elif redd > 3500 and yelloww > 3000:
            predicted_label = [0, 0, 0, 1, 0]
        elif max(redd, yelloww, greenn) == redd:
            predicted_label = [0, 1, 0, 0, 0]
        elif max(redd, yelloww, greenn) == yelloww:
            predicted_label = [0, 0, 1, 0, 0]
        else:
        ...
        if (predicted_label != true_label):
            # If these labels are not equal, the image has been misclassified
            misclassified_images_labels.append((im, predicted_label, true_label))
            print(str(redd) + '-' + str(yelloww) + '-' + str(greenn))
