...
class Tests(unittest.TestCase):
    ...
    def test_one_hot(self, one_hot_function):
        ...
        try:
            self.assertEqual([1, 0, 0, 0, 0], one_hot_function('0'))
            self.assertEqual([0, 1, 0, 0, 0], one_hot_function('1'))
            self.assertEqual([0, 0, 1, 0, 0], one_hot_function('2'))
            self.assertEqual([0, 0, 0, 1, 0], one_hot_function('3'))
            self.assertEqual([0, 0, 0, 0, 1], one_hot_function('4'))
        ...
        for im, predicted_label, true_label in misclassified_images:
            ...
            if (true_label == [0, 1, 0, 0, 0]):
                ...
                try:
                    ...
                    self.assertNotEqual(true_label, [0, 0, 0, 0, 1])
                except self.failureException as e:
                    # Print out an error message
                    print_fail()
