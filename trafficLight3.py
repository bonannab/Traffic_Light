...
for path, subdirs, files in os.walk("../scratch/fs2/DTLD_final/Bochum/Bochum1/2015-04-21_17-09-21/2"):
        for f in files:
            if os.path.isfile(os.path.join(path, f)):
                ...
                img = cv2.cvtColor(img, cv2.COLOR_BAYER_GB2RGB)####################34 out????
                #img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB) ################################# 26 out????
...
i = 0
while IMAGE_LIST[i][1] != 'green':
    i = i + 1
j = 0
while IMAGE_LIST[j][1] != 'yellow':
    j = j + 1
k = 0
while IMAGE_LIST[k][1] != 'red':
    k = k + 1
...
def get_misclassified_images(test_images):
    ...
    for image in test_images:
        ...
        if max(redd, yelloww, greenn) == redd:
            predicted_label = [1, 0, 0]
